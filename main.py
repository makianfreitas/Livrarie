...
def get_professores():
    conn = sqlite3.connect("livrarie.db")  # Conecta ao banco de dados
    cursor = conn.cursor()
    cursor.execute("SELECT IdProfessor, Nome, Matricula FROM Professor")
    rows = cursor.fetchall()
    conn.close()
    return rows
            ...
            disciplina_list.controls.append(
                ft.Container(
                    content=ft.ListTile(
                        ...
                        subtitle=ft.Text('Professor(a): Larissa', size=20, color=ft.colors.BLUE_200),
                        data=disciplina[0],
            ...
            [
                ...
                ft.Column(
                    [
                        ...
                        ft.Text(value=get_unique_disciplina_descricao(materiaAtual), size=30, color=ft.colors.BLUE_200)
                    ],
                    expand=True  # Expande a Column principal para ocupar o espaço vertical da página
                )
            ]
